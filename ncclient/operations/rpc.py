...
class RPCReply(object):
    ...
    *huge_tree*: parse XML with very deep trees and very long text content
    ...
    def _parsing_hook(self, root):
        ...
        """check ele or xml is contains non UTF8 characters, return non UTF8 tags"""

        def traverse(root, msg: list):
            for child in root:
                if child.text and "�" in child.text:
                    localname = etree.QName(child).localname
                    root_localname = etree.QName(root).localname
                    msg.append(f"<{root_localname}><{localname}>{child.text}</{localname}></{root_localname}>")
                traverse(child, msg)

        if "�" in self._raw:
            tags=[]
            traverse(root, tags)
            self._non_utf8_tags = tags
    ...
    @property
    def non_utf8_tags(self):
        return self._non_utf8_tags
        ...
        if self._async:
            ...
            if self._event.is_set():
                ...
                if self._device_handler.transform_reply():
                    return NCElement(self._reply, self._device_handler.transform_reply(), huge_tree=self._huge_tree), req
                else:
